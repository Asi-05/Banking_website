...
def test_set_budget_negative_limit_returns_error_string():
    ...
    with patch(
        "src.ui.controllers.budget_controller.budget_service.set_budget",
        side_effect=ValueError("Limit muss positiv sein"),
    ):
        result = controller.set_budget(payload)
...
def test_check_budget_status_not_exceeded():
    controller = _make_controller()
    fake_status = {"is_exceeded": False, "current_spending": 300.0, "limit_amount": 500.0}

    with patch(
        "src.ui.controllers.budget_controller.budget_service.check_budget_status",
...
def test_check_budget_status_exceeded():
    controller = _make_controller()
    fake_status = {"is_exceeded": True, "current_spending": 600.0, "limit_amount": 500.0}
...
def test_check_budget_status_exactly_at_limit():
    controller = _make_controller()
    fake_status = {"is_exceeded": True, "current_spending": 500.0, "limit_amount": 500.0}
...
def test_check_budget_status_no_budget_returns_error_string():
    ...
    with patch(
        "src.ui.controllers.budget_controller.budget_service.check_budget_status",
        side_effect=ValueError("Kein Budget gefunden"),
    ):
        result = controller.check_budget_status(user_id=1, month=4, year=2026)

    assert isinstance(result, str)
    assert "Budget" in result or result != ""
